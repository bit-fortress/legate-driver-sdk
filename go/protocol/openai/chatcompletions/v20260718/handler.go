...
type ResponseBody interface {
	protocolResponseBody() ([]byte, error)
}
...
func Register(handler Handler) driver.ProtocolHandlerRegistration {
	return driver.NewProtocolHandlerRegistration(Contract, func(request driver.TextAttemptOpenRequest) (*driver.TextAttemptOpenResult, *driver.DriverError) {
		...
		result, failure := handler.OpenAttempt(OpenAttemptInput{
			BoundState: append([]byte(nil), request.BoundState...), Mode: invocation.Mode,
			Request: typedRequest, ProtocolMetadata: metadata,
...
func (adapter attemptAdapter) FinishSSE() (*driver.TextSSEFinishSuccess, *driver.DriverError) {
	...
	converted, failure := rawStreamResult((*StreamEventResult)(result))
	if failure != nil {
		return nil, failure
	}
...
func rawResponse(response ClientResponse) (*driver.ClientResponse, error) {
	...
	return &driver.ClientResponse{
		StatusCode: response.StatusCode, Headers: response.Headers,
		Body:    &driver.ProtocolPayload{ProtocolContract: Contract, MediaType: driver.MediaTypeJSON, JSON: body},
		Outcome: response.Outcome, Usage: response.Usage,
	}, nil
}
...
func rawStreamResult(result *StreamEventResult) (*driver.TextSSETransformEventSuccess, *driver.DriverError) {
	...
	for _, event := range result.Events {
		...
		events = append(events, driver.ProtocolEventPayload{
			ProtocolContract: Contract, EventType: decoded.EventType, JSON: append([]byte(nil), decoded.JSON...),
		})
	}
